...
$Title = 'SDRS Automation Override: Policy Enforcement'

# Test description: How New-VesterConfig explains this value to the user
$Description = 'Specifies the behavior of SDRS when it generates recommendations for correcting affinity rule violations in a datastore cluster'

# The config entry stating the desired values
$Desired = $cfg.dscluster.autooverridepolicyenf
...
[ScriptBlock]$Actual = {
    $Object.ExtensionData.PodStorageDrsEntry.StorageDrsConfig.PodConfig.AutomationOverrides.PolicyEnforcementAutomationMode
}
...
[ScriptBlock]$Fix = {
    ...
    if($Desired -ne $NULL)
    {
        $Spec.PodConfigSpec.AutomationOverrides.PolicyEnforcementAutomationMode = $Desired
    }
